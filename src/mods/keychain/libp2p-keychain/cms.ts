...
export class CMS {
  ...
  constructor(keychain: KeyChain, dek: string) {
    if (keychain == null) {
      throw errCode(
        new Error('keychain is required'),
        codes.ERR_KEYCHAIN_REQUIRED
      )
    }
  ...
  async encrypt(name: string, plain: Uint8Array): Promise<Uint8Array> {
    if (!(plain instanceof Uint8Array)) {
      throw errCode(
        new Error('Plain data must be a Uint8Array'),
  ...
  async decrypt(cmsData: Uint8Array): Promise<Uint8Array> {
    if (!(cmsData instanceof Uint8Array)) {
      throw errCode(
        new Error('CMS data is required'),
        codes.ERR_INVALID_PARAMETERS
      )
    }
    ...
    if (r == null) {
      ...
      throw errCode(
        new Error(
          `Decryption needs one of the key(s): ${missingKeys.join(', ')}`
        ),
        codes.ERR_MISSING_KEYS,
        {
          missingKeys
        }
      )
