...
export class KeyChain {
  ...
  constructor(components: Components, init: KeyChainInit) {
    ...
    if (
      this.init.dek?.keyLength != null &&
      this.init.dek.keyLength < NIST.minKeyLength
    ) {
      throw new Error(`dek.keyLength must be least ${NIST.minKeyLength} bytes`)
    }
    if (
      this.init.dek?.salt?.length != null &&
      this.init.dek.salt.length < NIST.minSaltLength
    ) {
      throw new Error(
        `dek.saltLength must be least ${NIST.minSaltLength} bytes`
      )
    }
    if (
      this.init.dek?.iterationCount != null &&
      this.init.dek.iterationCount < NIST.minIterationCount
    ) {
      throw new Error(
        `dek.iterationCount must be least ${NIST.minIterationCount}`
      )
    }

    const dek =
      this.init.pass != null && this.init.dek?.salt != null
        ? pbkdf2(
            this.init.pass,
            this.init.dek?.salt,
  ...
  async createKey(
    ...
    if (exists) {
      await randomDelay()
      throw errCode(
        new Error('Key name already exists'),
        codes.ERR_KEY_ALREADY_EXISTS
      )
    }
    ...
    switch (type.toLowerCase()) {
      case 'rsa':
        if (!Number.isSafeInteger(size) || size < 2048) {
          await randomDelay()
          throw errCode(
            new Error('Invalid RSA key size'),
            codes.ERR_INVALID_KEY_SIZE
          )
        }
        break
      default:
  ...
  async findKeyByName(name: string): Promise<KeyInfo> {
    if (!validateKeyName(name)) {
      await randomDelay()
      throw errCode(
        new Error(`Invalid key name '${name}'`),
        codes.ERR_INVALID_KEY_NAME
      )
    }
    ...
    try {
      ...
      log.error(err)
      throw errCode(
        new Error(`Key '${name}' does not exist.`),
        codes.ERR_KEY_NOT_FOUND
      )
    }
  }
  ...
  async renameKey(oldName: string, newName: string): Promise<KeyInfo> {
    if (!validateKeyName(oldName) || oldName === 'self') {
      await randomDelay()
      throw errCode(
        new Error(`Invalid old key name '${oldName}'`),
        codes.ERR_OLD_KEY_NAME_INVALID
      )
    }
    if (!validateKeyName(newName) || newName === 'self') {
      await randomDelay()
      throw errCode(
        new Error(`Invalid new key name '${newName}'`),
        codes.ERR_NEW_KEY_NAME_INVALID
      )
    }
    ...
    if (exists) {
      await randomDelay()
      throw errCode(
        new Error(`Key '${newName}' already exists`),
        codes.ERR_KEY_ALREADY_EXISTS
      )
    }
  ...
  async importKey(
    ...
    if (pem == null) {
      await randomDelay()
      throw errCode(
        new Error('PEM encoded key is required'),
        codes.ERR_PEM_REQUIRED
      )
    }
    const dsname = DsName(name)
    const exists = await this.components.getDatastore().has(dsname)
    ...
    try {
      ...
      await randomDelay()
      throw errCode(
        new Error('Cannot read the key, most likely the password is wrong'),
        codes.ERR_CANNOT_READ_KEY
      )
    }
  ...
  async importPeer(name: string, peer: PeerId): Promise<KeyInfo> {
    try {
      ...
      if (peer == null || peer.privateKey == null) {
        throw errCode(
          new Error('Peer.privKey is required'),
          codes.ERR_MISSING_PRIVATE_KEY
        )
      }
  ...
  async rotateKeychainPass(oldPass: string, newPass: string) {
    if (typeof oldPass !== 'string') {
      await randomDelay()
      throw errCode(
        new Error(`Invalid old pass type '${typeof oldPass}'`),
    ...
    if (typeof newPass !== 'string') {
      await randomDelay()
      throw errCode(
        new Error(`Invalid new pass type '${typeof newPass}'`),
        codes.ERR_INVALID_NEW_PASS_TYPE
      )
    }
    if (newPass.length < 20) {
      await randomDelay()
      throw errCode(
        new Error(`Invalid pass length ${newPass.length}`),
        codes.ERR_INVALID_PASS_LENGTH
      )
    }
    ...
    const newDek =
      newPass != null && this.init.dek?.salt != null
        ? pbkdf2(
            newPass,
            this.init.dek.salt,
            this.init.dek?.iterationCount,
    ...
    for (const key of keys) {
      ...
      batch.put(
        DsInfoName(key.name),
        uint8ArrayFromString(JSON.stringify(keyInfo))
      )
      await batch.commit()
    }
