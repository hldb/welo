...
describe('Graph', () => {
  ...
  const muteNode = ({ in: ni, out } = {}) => new Node({ ...initNode, in: ni || new Set(), out: out || new Set() })
  const missNode = ({ in: ni }) => new Node({ ...muteNode({ in: ni }), miss: true })
  const denyNode = ({ in: ni }) => new Node({ ...muteNode({ in: ni }), deny: true })
  ...
  describe('class', () => {
    ...
    describe('.clone', () => {
      ...
      it('returns a clone of a mutated graph', () => {
        const graph = Graph.init()
        const cid = nodes[0]
        const out = new Set([nodes[1], missing[0], denied[0]])
    ...
    describe('add', () => {
      ...
      it('adds a cid to a graph and references an unknown node', () => {
        const graph = Graph.init()
        const cid0 = nodes[0]
        const out0 = new Set([missing[0]])

        Graph.add(graph, cid0, out0)

        assert.equal(graph.size, 1)
        assert.equal(graph.nodes.size, 2)
        assert.deepEqual(graph.nodes.get(cid0.toString()), muteNode({ out: new Set([missing[0].toString()]) }))
        assert.deepEqual(graph.nodes.get(missing[0].toString()), missNode({ in: new Set([cid0.toString()]) }))
      ...
      it('adds a cid to a graph and references an exising node', () => {
        ...
        const cid1 = nodes[1]
        const out1 = new Set([cid0])

        Graph.add(graph, cid1, out1)

        assert.equal(graph.size, 2)
        assert.equal(graph.nodes.size, 2)
        assert.deepEqual(graph.nodes.get(cid0.toString()), muteNode({ in: new Set([cid1.toString()]) }))
        assert.deepEqual(graph.nodes.get(cid1.toString()), muteNode({ out: new Set([cid0.toString()]) }))
      ...
      it('adds a cid to a graph and references a missing node', () => {
        ...
        assert.deepEqual(graph.nodes.get(cid0.toString()), muteNode({ out: new Set([missing[0].toString()]) }))
        assert.deepEqual(graph.nodes.get(cid1.toString()), muteNode({ out: new Set([missing[0].toString()]) }))
        assert.deepEqual(graph.nodes.get(missing[0].toString()), missNode({ in: new Set([cid0.toString(), cid1.toString()]) }))
      ...
      it('adds a cid to a graph and references a denied node', () => {
        ...
        const cid2 = nodes[2]
        const out2 = new Set([cid0])

        Graph.add(graph, cid2, out2)

        assert.equal(graph.size, 2)
        assert.equal(graph.nodes.size, 3)
        assert.deepEqual(graph.nodes.get(cid0.toString()), denyNode({ in: new Set([cid1.toString(), cid2.toString()]) }))
        assert.deepEqual(graph.nodes.get(cid1.toString()), muteNode({ out: new Set([cid0.toString()]) }))
        assert.deepEqual(graph.nodes.get(cid2.toString()), muteNode({ out: new Set([cid0.toString()]) }))
    ...
    describe('miss', () => {
      ...
      it('overwrites a node for an existing cid in a graph', () => {
        ...
        assert.equal(graph.size, 1)
        assert.equal(graph.nodes.size, 2)
        assert.deepEqual(graph.nodes.get(cid0.toString()), missNode({ in: new Set([cid1.toString()]) }))
        assert.deepEqual(graph.nodes.get(cid1.toString()), muteNode({ out: new Set([cid0.toString()]) }))
      ...
      it('removes an orphaned node for an existing cid in a graph', () => {
        ...
        assert.equal(graph.size, 1)
        assert.equal(graph.nodes.size, 2)
        assert.deepEqual(graph.nodes.get(cid1.toString()), missNode({ in: new Set([cid2.toString()]) }))
        assert.deepEqual(graph.nodes.get(cid2.toString()), muteNode({ out: new Set([cid1.toString()]) }))
    ...
    describe('deny', () => {
      ...
      it('overwrites a node for an existing cid in a graph', () => {
        ...
        assert.equal(graph.size, 1)
        assert.equal(graph.nodes.size, 2)
        assert.deepEqual(graph.nodes.get(cid0.toString()), denyNode({ in: new Set([cid1.toString()]) }))
        assert.deepEqual(graph.nodes.get(cid1.toString()), muteNode({ out: new Set([cid0.toString()]) }))
      ...
      it('removes an orphaned node for an existing cid in a graph', () => {
        ...
        assert.equal(graph.size, 1)
        assert.equal(graph.nodes.size, 2)
        assert.deepEqual(graph.nodes.get(cid1.toString()), denyNode({ in: new Set([cid2.toString()]) }))
        assert.deepEqual(graph.nodes.get(cid2.toString()), muteNode({ out: new Set([cid1.toString()]) }))
  ...
  describe('instance', () => {
    ...
    describe('heads', () => {
      it('returns the heads of the graph as a Set of cids', () => {
        ...
        assert.deepEqual(graph.heads, new Set([cid0.toString(), cid1.toString()]))

        const cid2 = nodes[2]
        const out2 = new Set([cid0, cid1])
    ...
    describe('tails', () => {
      it('returns the tails of the graph as a Set of cids', () => {
        ...
        assert.deepEqual(graph.tails, new Set([cid0.toString(), cid1.toString()]))
    ...
    describe('get', () => {
      it('returns a Node if cid is a vertex in the graph', () => {
        ...
        assert.deepEqual(graph.get(cid0), muteNode({ out: new Set([cid1.toString()]) }))
        assert.deepEqual(graph.get(cid1), missNode({ in: new Set([cid0.toString()]) }))
      })
